...
async function listOrders(req, res) {
  ...
  const orders = await prisma.order.findMany({
    ...
    select: {
      ...
      //
      // The four rule-101 billing fields joined billNo here on 2026-09-08 (rule 108): the Parties
  ...
  const response = orders.map((o) => ({
    ...
    ...(req.user.role === 'OWNER'
      ? {
          ...
          // Rule 108's "Correct billing" control reads all five: preTaxAmount decides whether the
...
// PATCH /api/orders/:id/billing-correction — OWNER + PIN (rule 108, added 2026-09-08).
//
...
// to rules 23 and 101, not a loophole in either — see 05_BUSINESS_RULES.md rule 108 for the
...
// top of that frozen figure, which is the narrow thing rule 108 authorises.
//
...
async function correctOrderBilling(req, res) {
  ...
  // No-op detection (rule 108, 2026-09-08) — explicitly ALLOWED, not rejected, as a deliberate
