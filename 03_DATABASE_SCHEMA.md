...
**A second real addition, same pattern (rule 105, 2026-09-09):** the migration `20260909145648_add_party_state` `ALTER TABLE`d the real `Party` to add `state PartyState?` (enum below). **Nullable at the database level on purpose, even though state is required going forward** — every Party row that already existed has no state value, so a `NOT NULL` column would have failed this migration against live data. "Required" is enforced only in `POST /api/parties`'s own validation (rule 105), never in the schema; `PATCH /api/parties/:id` does not require it, so an existing party stays fully editable without being forced to backfill a state it may not have on file. No default, no backfill — see rule 105 for the full reasoning.
...
// The real, current (2026) list of 28 states + 8 union territories (rule 105). Fixed and closed
...
model Order {
  ...
  finalAmount        Decimal? // preTaxAmount after discount is applied, before GST. Deliberately left UNROUNDED (rule 109) — it is an intermediate, and rounding it too would round one order twice.
  actualPayable      Decimal? // finalAmount after GST, THEN rounded to the nearest whole rupee (rule 109, 2026-09-19) — the real amount owed, never trusted from the client
  // Rule 109 (2026-09-19). The signed difference (rounded actualPayable − the raw figure the
  ...
  // ever applied — every order billed before 2026-09-19 (rule 109 is forward-only, with no
...
model OrderLineItem {
  ...
  bundleId         String   // article+color only (Bundle), deliberately not a specific Stock row — WHICH Location fulfills this is decided later, at BILLING (rule 64's FIFO-across-locations), not at order time. (Was packing until 2026-08-17.)
  bundle           Bundle   @relation(fields: [bundleId], references: [id])
...
model OrderBillingCorrection {
  ...
  // alongside rule 109's two new columns; the model itself has existed in schema.prisma since
  ...
  // Rule 109 (2026-09-19). Null here carries real meaning and must not be read as 0: it says the
  // order's previous billing predates rule 109 and was stored UNROUNDED. Paired with a non-null
  // newRoundingAdjustment below, this row is what makes that one-time transition legible — see
  // rule 109's "forward-only, with one deliberate and stated exception".
  ...
  newActualPayable      Decimal  // rounded to the whole rupee since rule 109, same as Order.actualPayable
...
- **Stock is deducted at `Billed`, never at `Packed`** *(changed 2026-08-17 — this previously happened at Packed)*. `Placed → Packed` is a pure counting/staging step: it records `qtySetsPacked` and writes any `SHORT_PACKED` adjustment, but touches no `Stock` row and creates no `Transaction`. `Packed → Billed` is the commitment point where the FIFO-across-locations deduction, the `STOCK_OUT` `Transaction` rows (one per Location drawn from, linked via `Transaction.orderLineItemId`), and the hard lock above all happen together. Deduction is driven by each line's **`qtySetsPacked`** (what was actually counted), never `qtySetsRequested`. Packing is a step staff may legitimately redo — hanging an irreversible deduction there risked double-deducting on a re-pack; `Billed` is already the one transition nothing can act past, so a deduction there can only run once by construction. See `LEARNING_LOG.md`.
...
- **Cancellation is a flag, never a status** *(added 2026-08-18)*: `Order.isCancelled` / `OrderLineItem.isCancelled`, orthogonal to `Order.status` so the forward-only chain (rule 59) needs no new state. Settable only while the order is `Placed` or `Packed` — from `Billed` on, rule 23's lock applies. Cancelling **never** rewrites `qtySetsRequested`/`qtySetsPacked`; the original figures stay as the historical record. Cancelling an order does not cascade to its lines (the order flag is authoritative), and cancelling the last live line does not auto-cancel the order (that's a separate deliberate act). A cancelled order cannot be packed/billed/shipped; a cancelled line is never deducted at billing.
