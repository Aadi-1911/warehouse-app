...
Response: `[{ id, partyId, partyName, status, isCancelled, createdAt, packedAt, billedAt, shippedAt, cancelledAt, lineItemCount, totalValue }]`. Default order: newest first. `isCancelled` is added (2026-08-20) so an unfiltered caller can distinguish a cancelled order from an active one at the same status — a status-scoped call (`?status=`) never returns a cancelled row to begin with (see below), so this only matters to callers reading the general, unfiltered list.
