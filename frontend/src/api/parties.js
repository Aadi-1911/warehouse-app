...
// GET /api/parties -> [{ id, name, shopName, location, address, contact, gstNo, state, isActive }]
...
// 2026-08-18 — staff need to add a walk-in customer mid-order. `name` and `state` are the only
// required fields (rule 109, added 2026-09-09 — state joined name as required at creation; every
// other field stays optional per the schema's minimal Phase 1 form, rule 17: a walk-in/one-off
// party shouldn't need a full profile before it can be recorded).
export function createParty({ name, shopName, location, address, contact, gstNo, state }) {
  return apiFetch('/api/parties', {
    method: 'POST',
    body: { name, shopName, location, address, contact, gstNo, state },
  });
}
