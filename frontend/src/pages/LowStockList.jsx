...
//
// Grouping (collapsible factory accordion -> collapsible article accordion -> colour/location/qty
// rows) and the collapsed article header's "colour count + combined low-set total" summary are the
// exact same shape as the Owner Dashboard's Low Stock page — reused here via the same
// .dash-lowstock-*/.accordion-* classes (all standalone, no dashboard-shell dependency) rather than
// reinvented, just mounted on this screen's own mobile ScreenHeader/page shell instead of the
// dashboard's layout. Every row here is low by construction (already filtered), so the per-row
// badge carries the real quantity rather than a redundant "Low" flag — same reasoning as the
// dashboard version.
//
// Factory level restructured 2026-08-26 from a plain always-expanded heading to the same
// collapsible .accordion-section/.accordion-header pattern Live Stock's own factory layer already
// uses (§3.2's "collapsed by default" convention) — this screen and the dashboard version had
// drifted from Live Stock at this one layer, and are now consistent with it everywhere.
...
export default function LowStockList() {
  ...
  // screen's factory layer collapses exactly like Live Stock's does (§3.2's "collapsed by
  // default" convention), instead of the plain always-expanded heading this screen started with.
  ...
  //
  // productIsActive excluded here too (2026-08-28, confirmed by Aadi): an archived article is
  // deliberately hidden from the daily pickers (rule 85) specifically so nobody acts on it day to
  // day — showing it in the low-stock nag would be the exact thing archiving exists to prevent.
  // This is NOT the same call the reporting/visibility task made for Live Stock's archived
  // section or the dashboard's stock-value KPIs: those are "what do we own and what's it worth,"
  // where archived stock is still real value and must count. This is "what should someone go
                              ...
                              {article.rows.map((row) => (
                                <div key={row.key} className="dash-lowstock-row">
                                  ...
                                  {/* Real quantity, not a redundant "Low" flag — every row here is
                                      already low by construction (pre-filtered above), same
                                      reasoning as the dashboard version's own per-row badge. */}
