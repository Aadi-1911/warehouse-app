...
- **Category** — ✅ backend complete, including archive/reactivate (closes the rule 85 gap this doc previously flagged as missing). **🔲 Frontend for archive/reactivate is built but not yet committed** — in progress, sitting as uncommitted local changes as of this writing. The original Receive Stock picker + inline "+ create new" has been complete and committed since Phase 1.
- **Party** — ✅ Backend complete: `POST`/`GET /api/parties` plus deactivate/reactivate all exist and are committed. **🔲 No frontend at all** — no page, no API wrapper, nothing linked from Home. Party is reachable via the API but not through the app yet.
