...
- **Factory Revenue & Sold-vs-Sitting** — 🔲 **backend only.** `utils/factoryRevenue.js` (sibling to `utils/locationRevenue.js`, sharing its `SALE_TRANSACTION_WHERE` sale definition) plus `GET /api/factories/analytics/revenue?period=...` and `GET /api/factories/analytics/sold-vs-sitting`, both OWNER-only, tested via curl against the dev DB. No frontend screen/donuts exist yet — this is the Factory-side counterpart to the Locations page's revenue/profit donuts, not yet surfaced anywhere in the dashboard UI.
...
- **Transfer** — ✅ COMPLETE, backend (`POST`/`GET /api/transfers`, atomic paired legs, negative-stock guard) **and** frontend (own Home tile, Stock-row-based selection, no "+ create" by design). Confirms and closes what was originally rule 46's accepted Phase 1 limitation. **Idempotency added 2026-09-11 (rule 107):** `Transfer.idempotencyKey` (nullable, unique), a client-generated key per staged line reused on retry, a pre-flight replay check plus `P2002` race handling in `transferController.js`, and `backend/test-transfer-idempotency.mjs` (29 assertions, covering the retry, the concurrent race, and keyless back-compat). Closes a gap confirmed by investigation on 2026-09-10 — the endpoint had no protection against a retried line being applied twice when its original response was lost after a successful commit. **Migration `20260911090000_add_transfer_idempotency_key` is applied to preview, test, and Production (confirmed 2026-09-21).**
- **Zero-stock suppression** (rule 110) — **✅ Built 2026-09-18, for every surface the rule names.** New Order's colour picker (`GET /api/products/:id/valid-colors`) excludes a Bundle with no real stock at the query level (`stock: { some: { qtySets: { gt: 0 } } }`). Live Stock's active view, `LowStockList.jsx`, and `dashboard/LowStock.jsx` each exclude `qtySets === 0` rows client-side, on top of the still-unfiltered `GET /api/stock` (left alone on purpose — that endpoint is shared with ArticlePricing/Home/BillOrderDetail/`dashboard/LiveStock.jsx`, which rule 110 does not name). Transfer's source picker was already compliant before this work (pre-existing `qtySets > 0` client-side filter). Pack Order was investigated and confirmed not applicable — it lists placed `OrderLineItem`s, not a stock-browsing surface, and its shortfall number already treats a zero-quantity Stock row identically to no row at all. *(The companion whole-article auto-archive idea designed alongside this on 2026-09-09 — Article-level `wentOutOfStockAt`, an "Out of Stock" tab, auto-archive after a configurable threshold — has nothing built and no rule number on `main`; it is tracked on `staging` only.)*
...
- **Owner Desktop Dashboard status:** ✅ far beyond the shell-and-Overview state this line described as of 2026-08-19. OWNER-gated `/dashboard`, header, and the six-KPI Overview (`GET /api/dashboard/overview`) plus its recent-activity feed (`GET /api/history`, unchanged) are all still there as originally built. **Orders** (`889e453` onward), **Low stock** (`9342968`), **Parties** (`4cd6e23`), and **History** (`3c2f040`) are each fully built and committed, real multi-hundred-line screens, not placeholders. A dashboard-wide **PIN lock** also shipped (`e31c80e`) — a privacy gate over the whole `/dashboard` shell (entry gate, idle auto-relock, manual lock button; see `DashboardLayout.jsx`'s own comment for why it's a privacy gate, not a second security boundary), not the per-task "owner PIN-gate dialog" this line originally described as belonging to the Orders task. The rail has also grown from its original five items to nine: **Locations** (`e31c80e`), **Article Pricing** (`9e0ba65`), **Factory Payables** (`a08dfd4`), and **Bills** (`c157f3e`) are all real, committed dashboard pages beyond the original brief. *(This line previously described four pages as "named placeholders" as of 2026-08-19 — that stopped being true across a long series of follow-up tasks and was never updated here; corrected 2026-08-31.)* Still genuinely absent from Overview specifically, confirmed by reading `Overview.jsx`'s own comment: two of the design's three "extras" widgets (top parties, low-stock preview), and a New Order dialog/order drawer on the dashboard itself — New Order still lives only on mobile. The third — stock value by location — is no longer missing anywhere in the dashboard: it shipped on the **Locations** page instead (`76142aa`), alongside two further donuts (revenue by location, profit by location) covering the same per-location comparison the Overview extra was meant to provide, not on Overview itself.
- **Feature:** Live order capture during in-shop sample visits, plus the manual "Log Order from Photo" fallback.
- **Billing fulfilment location:** ✅ built 2026-09-07. `PATCH /api/orders/:id/bill` now requires `locationId` + `locationConfirmed: true` and deducts stock from that ONE location only — replacing the previous alphabetical walk across every location, which could silently split a single order line's quantity across two of them. Adds OWNER-only `GET /api/orders/:id/fulfillment-preview?locationId=...` (per-line needed-vs-available, sharing `utils/orderFulfillment.js`'s check with the real deduction rather than reimplementing it), plus a GGN/Delhi toggle, live preview, and required confirmation checkbox on BOTH billing entry points (mobile Bill Order and the dashboard Orders "Mark billed" modal, via the shared `components/BillFulfillmentPicker.jsx`). Forward-only: historical `Transaction` rows written under the old logic were deliberately left untouched.
- **Whole-rupee rounding of `actualPayable`:** ✅ built 2026-09-19 (rule 109). Rule 101's discount-then-GST arithmetic is unchanged; `actualPayable` is now rounded to the nearest whole rupee afterwards (standard round-half-up, Excel's `ROUND()`, explicitly not banker's), with the signed difference stored in the new `Order.roundingAdjustment` so the rounding is auditable rather than silently absorbed. `finalAmount` stays unrounded on purpose. Applied in `utils/orderBillingAmounts.js`, the one function both `billOrder()` and `correctOrderBilling()` already share, so neither write path can drift from the other. Adds `OrderBillingCorrection.oldRoundingAdjustment`/`newRoundingAdjustment`, a "Rounding" line on the Owner Dashboard's expanded order row (shown only when non-zero), and `backend/test-order-rounding.mjs` (41 assertions — the tie direction, a negative adjustment, the real Production shape, corrections on both rounded and pre-rule-109 orders, and a verified forward-only check). Forward-only: existing billed orders keep their unrounded figure, with one stated exception — correcting a pre-rule-109 order's billing does round it, since the correction endpoint recomputes through the shared function. **Migration `20260919090000_add_order_rounding_adjustment` is applied to preview, test, and Production (confirmed 2026-09-21).**
