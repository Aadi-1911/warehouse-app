...
- **Out-of-stock suppression & whole-article auto-archive** — **🔲 Designed only, nothing built.** No migration applied, no endpoints, no screen — unlike the item above, not even the schema exists yet. Two agreed rules, added 2026-09-09: **rule 107** hides any individual `Stock` row at `qtySets = 0` from Live Stock, both Low Stock screens, and every stock-listing picker (immediate, per-row, purely a browsing filter — nothing deleted); **rule 108** tracks an Article that hits zero across *every* colour and location via a new `Product.wentOutOfStockAt`, lists those in a new "Out of Stock" tab, and auto-archives them through rule 85's existing Article archive once a configurable threshold (default 60 days) passes. Needs `Product.wentOutOfStockAt` plus a new general-purpose `AppSetting` key-value table for the owner-editable threshold — both specced in `03_DATABASE_SCHEMA.md` §1.2, deliberately parked there rather than in §1 until migrated.
