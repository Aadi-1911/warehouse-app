...
- **Out-of-stock suppression & whole-article auto-archive** — two agreed rules, added 2026-09-09, now at different build stages:
  - **Rule 107** (hides any individual `Stock` row at `qtySets = 0` from Live Stock, both Low Stock screens, and every stock-listing picker) — **✅ Built 2026-09-18, for every surface it names.** New Order's colour picker (`GET /api/products/:id/valid-colors`) now excludes a Bundle with no real stock at the query level (`stock: { some: { qtySets: { gt: 0 } } }`). Live Stock's default view, `LowStockList.jsx`, and `dashboard/LowStock.jsx` each exclude `qtySets === 0` rows client-side, on top of the still-unfiltered `GET /api/stock` (left alone on purpose — that endpoint is shared with ArticlePricing/Home/BillOrderDetail/`dashboard/LiveStock.jsx`, which rule 107 doesn't name). Transfer's source picker was already compliant before this work (pre-existing `qtySets > 0` client-side filter). Pack Order was investigated and confirmed not applicable — it lists placed `OrderLineItem`s, not a stock-browsing surface, and its shortfall number already treats a zero-quantity Stock row identically to no row at all.
  - **Rule 108** (Article-level `wentOutOfStockAt` tracking, an "Out of Stock" tab, auto-archive via rule 85 after a configurable threshold) — **🔲 Still designed only, nothing built.** No migration applied, no endpoints, no screen, not in progress. Needs `Product.wentOutOfStockAt` plus a new general-purpose `AppSetting` key-value table for the owner-editable threshold — both specced in `03_DATABASE_SCHEMA.md` §1.2, deliberately parked there rather than in §1 until migrated.
